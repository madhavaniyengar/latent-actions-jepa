...
class MaskCollator(object):
    ...
    def __call__(self, batch):
        ...
        collated_masks_pred, collated_masks_enc, collated_p = [], [], []
        for i, mask_generator in enumerate(self.mask_generators):
            masks_enc, masks_pred, p = mask_generator(batch_size)
            collated_masks_enc.append(masks_enc)
            collated_masks_pred.append(masks_pred)
            collated_p.append(p)

        return collated_batch, collated_masks_enc, collated_masks_pred, collated_p
...
class _MaskGenerator(object):
    ...
    def _sample_block_mask_before_p(self, b_size, p):
        t, h, w = b_size
        top = torch.randint(0, self.height - h + 1, (1,))
        left = torch.randint(0, self.width - w + 1, (1,))
        start = torch.randint(0, self.duration - t + 1, (1,))
    ...
    def __call__(self, batch_size):
        ...
        for _ in range(batch_size):
            ...
            while empty_context:
                ...
                for _ in range(self.npred):
                    ...
                    mask_e *= self._sample_block_mask_before_p(p_size, p)
                mask_e = mask_e.flatten()

                # make mask e zero after index p
                # mask_e[p:] = 0
        ...
        return collated_masks_enc, collated_masks_pred, p
